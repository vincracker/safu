    ...
    <div className="font-Lato border-2 border-primary rounded-2xl md:w-[40%] mx-auto  mt-6">
      ...
      {orders != undefined &&
        ...
        (btnSelected === "Sending" ? (
                          ...
                          <div className="space-y-2">
                            <div className="flex items-center gap-x-3">
                              <img
                                src={
                                  tokenslist.filter(
                                    (token) => token.address.toUpperCase() == order.tokenAddress.toUpperCase()
                                  ).length > 0
                                    ? tokenslist.filter(
                                        (token) => token.address.toUpperCase() == order.tokenAddress.toUpperCase()
                                      )[0].logoURI
                          ...
                          <div className="flex space-x-2 justify-between">
                            <div className="flex items-center gap-x-3">
                              ...
                              <img
                                src={
                                  tokenslist.filter(
                                    (token) => token.address.toUpperCase() == order.swapTokenAddress.toUpperCase()
                                  ).length > 0
                                    ? tokenslist.filter(
                                        (token) => token.address.toUpperCase() == order.swapTokenAddress.toUpperCase()
                                      )[0].logoURI
          ...
          <div className="h-64 p-4 overflow-y-scroll">
            {orders
              .filter((order: Order) => order.receiver == account.toLowerCase() && order.status.toString() == "0")
              .filter((order: Order) => (location.pathname == "/app" ? order.isSwap == false : order.isSwap == true))
              .length > 0 ? (
              orders
                          ...
                          <div className="flex space-x-2 justify-between">
                            ...
                            <div className="space-y-2 text-end">
                              ...
                              {localTime <= order.swapDeadline ? (
                                ...
                                <button className="bg-[#3a3a3a] text-white px-4 py-0.5 rounded-sm cursor-not-allowed">
                                  Expired
                                </button>
                              )}
                            </div>
        ...
        {!validPP ? (
          <form className="w-full" onSubmit={formik.handleSubmit}>
            <div className="w-full flex flex-col items-center">
              ...
              <br />
              <div className="w-4/5 flex justify-between p-x-4 gap-x-5">
                <button
                  className="w-1/2 flex items-center justify-center bg-slate-500 text-white rounded-md font-bold text-xl py-2 px-4"
                  onClick={() => {
                    cancelClaim();
                  }}
                >
                ...
                {!loading ? (
                  error.length > 0 ? (
                    <button
                      className="w-1/2 flex items-center justify-center bg-primary text-white rounded-md font-bold text-xl py-2 px-4 cursor-not-allowed"
                      type="button"
                    >
                    ...
                    <button
                      className="w-1/2 flex items-center justify-center bg-primary text-white rounded-md font-bold text-xl py-2 px-4"
                      type="submit"
                    >
                      {approved ? "Confirm" : "Approve"}
                    </button>
                  ...
                  <button className="w-1/2 flex items-center justify-center bg-primary text-white rounded-md font-bold text-xl py-2 px-4">
                    <div role="status">
                      <svg
                        aria-hidden="true"
                        className="w-8 h-8 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
                ...
                {order.isSwap ? (
                  <div className="text-center">
                    <p>
                      ...
                      {tokenslist.filter((token) => token.address.toUpperCase() == order.swapTokenAddress.toUpperCase())
                        .length > 0
                        ? tokenslist.filter(
                            (token) => token.address.toUpperCase() == order.swapTokenAddress.toUpperCase()
                          )[0].symbol
                      ...
                      {tokenslist.filter((token) => token.address.toUpperCase() == order.tokenAddress.toUpperCase())
                        .length > 0
                        ? tokenslist.filter(
                            (token) => token.address.toUpperCase() == order.tokenAddress.toUpperCase()
                          )[0].symbol
                    ...
                    <p>
                      with {order.sender.slice(0, 5)}...
                      {order.sender.slice(order.sender.length - 5, order.sender.length)} as your counterparty
                    </p>
                    <p>
                  ...
                  <div className="text-center">
                    ...
                    <p>
                      Transaction fee {ethers.utils.formatEther(BigNumber.from(order.amount).mul(fee).div(10000))}{" "}
                      {tokenslist.filter((token) => token.address.toUpperCase() == order.tokenAddress.toUpperCase())
                        .length > 0
                        ? tokenslist.filter(
                            (token) => token.address.toUpperCase() == order.tokenAddress.toUpperCase()
              ...
              <div className="w-full flex flex-col items-center gap-4">
                ...
                {order.isSwap ? (
                  <p>
                    ...
                    {tokenslist.filter((token) => token.address.toUpperCase() == order.tokenAddress.toUpperCase())
                      .length > 0
                      ? tokenslist.filter((token) => token.address.toUpperCase() == order.tokenAddress.toUpperCase())[0]
                          .symbol
                      : "UNK"}{" "}
                  </p>
                ) : (
          ...
          <>
            <div className="w-full flex flex-col items-center">
              <img src={logo} alt="logo" className="w-20" />
              <p>{order.isSwap ? "Your transfer" : "Your swap"} has been completed</p>
              <p>
                TxHash:{" "}
                <a
